...
def generateGlslTransitionSource (filepath)
  ...
  "sliderjs.glslTransitions.register('"+name+"', sliderjs.util.extend("+opts+", {\n"+
  "  shader: '"+r.split("\n").join("\\n").split("'").join("\\'")+
  "'\n"+
  "}));\n"
end
...
def slider_js_files
  Dir['src/utils.js', 'src/core.js', 'src/SelectorTemplating/SelectorTemplating.js', 'src/modules/*.js', 'src/transitions/canvas/*.js']
end
