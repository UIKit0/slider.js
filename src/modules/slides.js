//~~~ Slides
sliderjs.modules.register("slides", function (sandbox, $) {
  ...
  function onSlideChanged (o) {
    setTimeout(function(){
      syncSlide(o.value, o.old); // differed DOM update to permit cssTransition to work fine. TODO: BTW maybe this should go to css transition (and the "current" class)
    }, 200);
  }
  ...
  function load (slides) {
    ...
    images = $.map(slides, function (slide, i){
      var img = new Image();
      $.bind(img, "load",  function () { handle("success", i, img, id) });
      $.bind(img, "error", function () { handle("error", i, img, id) });
      $.bind(img, "abort", function () { handle("abort", i, img, id) });
      img.src = slide.image;
      return img;
    });
  }
