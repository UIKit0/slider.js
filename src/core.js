...
ns.Slider = function (container, options) {
  ...
  self.opt = function (optname, newValue) {
    ...
    if (modules[optname])
      newValue = modules[optname].m.fix(newValue, value);
    ...
    return {
      ...
      start: function (names) {
        $.each($.getList(names), function (name) {
          var moduleFunction = ns.modules.get(name), onChanged, module;
          ...
          module = $.extend({ //extend the module with module defaults
            ...
            change: $.nop,
            destroy: $.nop,
            fix: function (value) { return value },
            fn: function (val) {
              var args = [name]; 
          ...
          // Set default
          options[name] = module.fix(name in options ? options[name] : module.def);
  ...
  // start all modules
  var modulesList = ns.modules.list();
